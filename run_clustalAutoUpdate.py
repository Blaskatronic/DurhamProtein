...
#		print truncatedMatrix
		...
		self.groupAnalysis()
		paramsListElement, loopIndexElement, alpha, beta = self.output()
		return paramsListElement, identity, loopIndexElement, alpha, beta
...
#		print "\n\n\n"
		...
		if self.groupUserDefined != None:
			...
			print "And the user defined group identity percentages are:", customidentity, "- a conservation factor of %.2f%%," % (customConservation)
			self.params.append(ligandConservation)
			self.params.append(customConservation)
		else:
			print "And the ligand binding site identity percentages are:", ligandidentity, "- a conservation factor of %.2f%%," % (ligandConservation)
			self.params.append(ligandConservation)
			self.params.append(None)
		self.identifyLoops()
...
#		print self.groupBetaSheet
	...
	def output(self):	# Output the required data back to the main program (maybe pickle it and then have a seperate program for analysis comparison?)
		return self.params, self.loopIndices, self.groupAlphaHelix, self.groupBetaSheet

class analysis:
	def __init__(self, paramsList, sortFactor1, mutationalityList, loopIndexElement, alpha, beta):
...
#			P.ylabel('Conservation Factor in inputted Sequences')
	...
	def structure(self):
		structureMatrix = np.zeros((10, 210))
#		print self.alpha
#		print self.beta
		for section in self.alpha:
			structureMatrix[0:10, section[0]:section[1]] = 0.3
			width = section[1]-section[0]
			if width >= 8:		# Draw an Alpha
				indent = section[0]+int((width-8)/2)
				structureMatrix[5, indent+1] = 2
				structureMatrix[4, indent+1] = 2
				structureMatrix[6, indent+1] = 2
				structureMatrix[3, indent+2] = 2
				structureMatrix[7, indent+2] = 2
				structureMatrix[3, indent+3] = 2
				structureMatrix[7, indent+3] = 2
				structureMatrix[4, indent+4] = 2
				structureMatrix[6, indent+4] = 2
				structureMatrix[5, indent+5] = 2
				structureMatrix[4, indent+6] = 2
				structureMatrix[6, indent+6] = 2
				structureMatrix[3, indent+7] = 2
				structureMatrix[7, indent+7] = 2
		for section in self.beta:
			structureMatrix[0:10, section[0]:section[1]] = 3
			width = section[1]-section[0]
			if width >= 6:		# Draw a Beta
				indent = section[0]+int((width-5)/2)
				structureMatrix[1:9, indent+1] = 2
				structureMatrix[2, indent+2] = 2
				structureMatrix[5, indent+2] = 2
				structureMatrix[8, indent+2] = 2
				structureMatrix[2, indent+3] = 2
				structureMatrix[5, indent+3] = 2
				structureMatrix[6, indent+3] = 2
				structureMatrix[8, indent+3] = 2
				structureMatrix[3, indent+4] = 2
				structureMatrix[4, indent+4] = 2
				structureMatrix[7, indent+4] = 2
...
#		print allosteryValues
		...
		ax0b.plot(xvals, self.mutationalityList, '-k', linewidth=0.8)
...
def CLI(paramsList, identityList, loopIndexElement, alpha, beta, totalSequences):
	print "\n---------------===============***************===============---------------"
	print "\nRESULTS"
	print "\n---------------===============***************===============---------------\n"
	sortFactor1 = 0
...
if __name__ == "__main__":
	...
	for element in orderedList:
		paramsListElement, identity, loopIndexElement, alpha, beta = sequences(element).runAnalysis()
