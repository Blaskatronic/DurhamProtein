...
class clustalw:
	...
	def checkAlign(self, name):
		...
		nameAln = name[:-3]+'aln'
		check = os.path.exists(nameAln)   # Windows
	...
	def runClustal(self):
		...
		for element in self.listSeq:
			...
			if check == True and self.overwriteFlag == 0:
...
#		self.groupAnalysis() # NO LONGER NEEDED UNLESS EXPLORING CAP
#		paramsListElement, loopIndexElement, alpha, beta, custom = self.output() # CAP stuff
                paramsListElement = self.output()
#		return paramsListElement, identity, loopIndexElement, alpha, beta, custom # CAP stuff
		return paramsListElement, identity
...
#		return self.params, self.loopIndices, self.groupAlphaHelix, self.groupBetaSheet, self.groupUserDefined # Includes group stuff
                return self.params
...
#			P.ylabel('Conservation Factor in inputted Sequences')
	...
	#### THERE MUST BE A WAY TO SOFT CODE THIS NUMBER IN IN ORDER TO FIX IT TO THE LENGTH OF THE COMPARATOR SEQUENCE DIRECTLY
		xvals = list(np.arange(1, 329))
	...
	#			wtallostery.append(wtvalue)
		if mode == 'valsOnly':
			return allosteryResidues, allosteryValues, mutationVals
                P.close()
#		P.ion()
		fig = P.figure(figsize = (18, 10))
#		print self.mutationOccurence
#		print np.sum(self.mutationOccurence.values())
		xvals = np.arange(1,len(mutationVals)+1)
		yvals = mutationVals
		P.plot(xvals, yvals, 'xb')
		P.xlim(0,np.max(xvals))
		P.ylim(0,np.max(yvals))
#		P.xlim(0, 210)
		P.xlabel('Residue Number')
		P.ylabel('Mutation Factor')
		P.show()

	def allostery2(self, mode='noLoop', barlinewidth=None):
		allosteryReader = csv.reader(open('allval.csv', 'rb'))
		allosteryResidues = []
		allosteryValues = []
		wtallostery = []
...
#def sortParams(paramsList, identityList, loopIndexElement, alpha, beta, cutOff, sortFactor1, totalSequences): # CAP stuff
def sortParams(paramsList, identityList, cutOff, sortFactor1, totalSequences):
...
#	return newParams, mutationalityList, loopIndexElement, alpha, beta # CAP Stuff
        return newParams, mutationalityList
